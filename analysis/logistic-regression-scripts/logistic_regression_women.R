...
m_women.stereotyped <- glm(
  genai_p_women ~ I(bls_p_women - 0.5),
  family = quasibinomial,
  data   = proportions,
  weights = genai_n
)

pdf("logreg_women.pdf", width = 8, height = 6)
par(
  cex.main = 1.5,
  cex.lab  = 1.5,
  cex.axis = 1.3,
...
xlim <- c(0, 1)
ylim <- c(0, 1)
max_bls <- max(proportions$bls_p_women, na.rm = TRUE)

# --- Set up empty plot ---
plot(NA, xlim = xlim, ylim = ylim,
     xlab = "BLS proportion women",
     ylab = "Gemini 2.5 proportion women",
...
vr <- visreg(
  m_women.stereotyped, "bls_p_women",
  scale = "response",
  plot  = FALSE
)
