...
m_white.stereotyped <- glm(
  genai_p_white ~ I(bls_p_white - 0.5),
  family = quasibinomial,
  data   = proportions,
  weights = genai_n
)

pdf("logreg_white.pdf", width = 8, height = 6)
par(
  cex.main = 1.5,
  cex.lab  = 1.5,
  cex.axis = 1.3,
...
xlim <- c(0, 1)
ylim <- c(0, 1)
max_bls <- max(proportions$bls_p_white, na.rm = TRUE)

# --- Set up empty plot ---
plot(NA, xlim = xlim, ylim = ylim,
     xlab = "BLS proportion white",
     ylab = "Gemini 2.5 proportion white",
...
vr <- visreg(
  m_white.stereotyped, "bls_p_white",
  scale = "response",
  plot  = FALSE
)
